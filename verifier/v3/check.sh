#!/bin/bash
# verifier v3：瓦片分片重构验收（适配对象存储单目录容量限制）
cd "$(dirname "$0")/../.." || exit 1
PASS=0; FAIL=0
...
MC=0; for t in $MPROBE; do [ -s "public/tiles-modern/gaode/z${t%%-*}/${t}.png" ] && MC=$((MC+1)); done
...
for layer in tang nsong ssong; do
  # z12 分片：x 偶数 → s0，奇数 → s1
  [ -s "public/tiles/${layer}/z12/s0/12-3276-1615.png" ] && AC=$((AC+1))
  [ -s "public/tiles/${layer}/z12/s0/12-3362-1643.png" ] && AC=$((AC+1))
  [ -s "public/tiles/${layer}/z12/s1/12-3333-1732.png" ] && AC=$((AC+1))
  [ -s "public/tiles/${layer}/z11/11-1664-866.png" ] && AC=$((AC+1))
done
[ "$AC" -eq 12 ] && ok "古地图 z11/z12 分片探测 12/12" || bad "古地图探测 ${AC}/12"
AS=$(du -sm public/tiles 2>/dev/null | cut -f1)
[ "${AS:-0}" -ge 30 ] && ok "古地图体积 ${AS}MB(≥30)" || bad "古地图体积 ${AS}MB"
...
grep -q "shiluFb" src/components/MapView.tsx && ok "本地缺失自动回源自愈" || bad "回源自愈缺失"
grep -q "public/tiles/" .gitignore && ok "瓦片已移出 git（目录容量限制）" || bad "gitignore 未配置"
